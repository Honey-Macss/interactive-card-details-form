...
export const SchemaWay = yup.object().shape({
    ...
    expiryDateMm: yup.number().positive().integer().min(1, "Expiry month must be at least 1 month").max(12, "Expiry month can not be more than 12 months").required("Can't be blank"),
    expiryDateYy: yup.number().positive().integer().min(currentYear, `Expiry year must be at least ${currentYear}`).required("Can't be blank "),
